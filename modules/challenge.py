...
def find_challenge(db_guild: dict, challenge_id: int):
    ...
    Args:
        ...
        challenge_id (int): The id of the target challenge document.

    Returns:
        The target challenge document. Otherwise, returns None.
    """
    guild_challenges = db_guild["challenges"]
    result = [
        challenge for challenge in guild_challenges if challenge["id"] == challenge_id
    ]
    if result:
        return result[0]
    return None
...
def find_active_challenge_by_user(db_guild: dict, user_id: int):
    ...
    Returns:
       The target challenge if successful. Otherwise, returns None.
    """
    try:
        return list(
            filter(
...
async def create_challenge(
    interaction: Interaction, player_mention: str | None, best_of: int
):
    """Creates a new challenge match between two players.
    Adds the challenge document and sends the challenge message in the interaction channel.
    If player_mention is None, waits for a challenger.

    Args:
        interaction (Interaction): The discord command interaction.
        player_mention (str | None): The target player challenge [<@player_id>].
        best_of (int): The max number of matches. Must be an positive odd integer.

    Returns:
        The new challenge document if successful. Otherwise, returns None.
    """
    ...
    if active_challenge:
        await interaction.followup.send(
            "You already have an active challenge.", ephemeral=True
        )
        return None
    ...
    if best_of % 2 != 1 or best_of < 1:  # must be odd
        await interaction.followup.send(
            "There must be an positive odd number of rounds.", ephemeral=True
        )
        return None
    ...
    if player_mention is not None and len(player_mention.strip()) > 0:
        ...
        if matched_id:
            player2: Member = await guild.fetch_member(
                int(player_mention[3:-1])
            )  # TODO: what if the player doesn't exist
        else:
            await interaction.followup.send(
                f"Invalid player mention.\n{usage}", ephemeral=True
            )
            return None
    else:
        player2: Member = None
    ...
    new_challenge = {
        ...
        "player2": {"id": player2_id, "name": player2_name, "vote": None},
        "best_of": best_of,
        "winner_emote": None,
    ...
    challenge_message: Message = await channel.send(
        content=f"<@!{player2.id}> has been challenged by <@!{player1.id}>!"
        if player_mention
        else "",
        embed=embed,
        view=accept_view(),
    ...
    try:
        ...
        printlog(
            f"Failed to add challenge ['id'='{challenge_message.id}'] to guild ['guild_id'='{guild.id}'].",
            e,
        )
        ...
        return None
    print(
        f"User '{player1.name}' ['id'='{player1.id}'] created new challenge ['id'='{new_challenge['id']}']."
    )
    await interaction.followup.send(
        f"Successfully created new challenge.", ephemeral=True
    )
    return new_challenge


async def cancel_challenge(
    interaction: Interaction, challenge_id: int, delete: bool = False
) -> bool:
    """Cancels an open challenge match. and deletes the channel message.
    If delete option is True, deletes challenge regardless of status. Only available to guild admins or tournament managers.

    Args:
        interaction (Interaction): The Discord command interaction.
        challenge_id (int): The id of target challenge.
        delete (bool, optional): A flag to determine whether to delete the challenge in the database. Defaults to False.
    ...
    # Fetch challenge
    usage = (
        "Usage: `$challenge cancel [name]` or `$challenge delete [name]` (admins only)"
    )
    db_challenge, challenge_id = await retrieve_valid_challenge(
        interaction, db_guild, challenge_id
    )
    if not db_challenge:
        return False
    # Cancel or Delete
    if not delete:
        if (
            user.id != db_challenge["player1"]["id"]
            or not user.guild_permissions.administrator
        ):
            await interaction.followup.send(
                f"Only the author or server admins can cancel challenges.",
                ephemeral=True,
            )
            return False
    else:
        if not user.guild_permissions.administrator:
            await interaction.followup.send(
                f"Only server admins can delete completed challenges.", ephemeral=True
            )
            return False
    # Check if in channel challenge was created in
    if db_challenge["channel_id"] != channel.id:
        await interaction.followup.send(
            f"Must be in the channel that the challenge was created in: <#{db_challenge['channel_id']}>",
            ephemeral=True,
        )
        return False
    ...
    if not delete:
        if db_challenge["accepted"] or db_challenge["completed"]:
            await interaction.followup.send(
                f"You may not cancel challenges that are in progress or completed.",
                ephemeral=True,
            )
            return False
    ...
    if not result:
        ...
        return False
    print(
        f"User '{user.name}' [id={user.id}] cancelled/deleted challenge ['id'='{challenge_id}']."
    )
    await interaction.followup.send(
        "Challenge has been successfully cancelled.", ephemeral=True
    )
    ...
    if delete and db_challenge["completed"]:
        ...
        if db_challenge["winner_emote"] == "1️⃣":
            db_winner: dict = _leaderboard.find_leaderboard_user(
                db_guild, db_challenge["player1"]["name"]
            )
            db_loser: dict = _leaderboard.find_leaderboard_user(
                db_guild, db_challenge["player2"]["name"]
            )
        else:
            db_winner = _leaderboard.find_leaderboard_user(
                db_guild, db_challenge["player2"]["name"]
            )
            db_loser = _leaderboard.find_leaderboard_user(
                db_guild, db_challenge["player1"]["name"]
            )
        # Update records
        db_winner["matches"] = list(
            filter(
                lambda match_id: match_id != db_challenge["id"], db_winner["matches"]
            )
        )
        db_winner.update({"wins": db_winner["wins"] - 1})
        db_loser["matches"] = list(
            filter(
                lambda match_id: match_id != db_challenge["id"], db_winner["matches"]
            )
        )
...
async def accept_challenge(interaction: Interaction):
    """Allows a user to accept an open challenge.
    ...
    if db_challenge["player1"]["id"] == user.id:
        await interaction.followup.send(
            "You cannot accept your own challenge.", ephemeral=True
        )
        return False
    # Check if completed
    if db_challenge["completed"]:
        await interaction.followup.send(
            "This challenge has already been completed.", ephemeral=True
        )
        return False
    ...
    if db_challenge["player2"]["id"] is not None:  # Direct challenge
        ...
        if db_challenge["player2"]["id"] != user.id:
            await interaction.followup.send(
                "You are not the user being challenged.", ephemeral=True
            )
            return False
        ...
        if db_challenge["accepted"]:
            await interaction.followup.send(
                "This challenge has already been accepted.", ephemeral=True
            )
            return False
        ...
        if not db_challenge["open"]:
            await interaction.followup.send(
                "This challenge is currently not open.", ephemeral=True
            )
            return False

        # Add user to challenge
        db_challenge["player2"] = {"id": user.id, "name": user.name, "vote": None}

    # Set to closed
    db_challenge.update(
        {"open": False, "accepted": datetime.now(tz=pytz.timezone("US/Eastern"))}
    )
    ...
    if not result:
        ...
        return False
    print(
        f"User ['name'='{user.name}'] accepted challenge ['id'='{db_challenge['id']}'] by '{db_challenge['player1']['name']}'."
    )

    # Update message embed and buttons
    updated_embed = edit_challenge_embed_start(
        challenge_message.embeds[0], db_challenge
    )
    await challenge_message.edit(embed=updated_embed, view=voting_buttons_view())
    await interaction.followup.send(
        f"You have accepted the challenge by <@{db_challenge['player1']['id']}>!",
        ephemeral=True,
    )
    return True
...
async def vote_challenge_button(interaction: Interaction, button: Button):
    ...
    Args:
        interaction (Interaction): The discord command interaction.
        button (Button): The voting discord button.
    ...
    return await _match.vote_button(
        interaction, button, challenge_message, db_guild, db_challenge
    )
...
async def report_challenge(
    ...
    Args:
        challenge_message (Message): The discord message for the challenge.
        db_guild (dict): The discord guild database document.
        db_challenge (dict): The challenge database document.
        winner_emote (str): The winner emote. Either '1️⃣' or '2️⃣'.
        is_forfeit (bool, optional): Whether the result is a forfeit or not. Defaults to False.
    ...
    confirm_embed = _match.edit_match_embed_confirmed(
        challenge_embed,
        challenge_id,
        db_challenge["player1"],
    ...
    print(
        "Succesfully reported challenge [id={0}]. Winner = '{1}'.".format(
            challenge_id, winner["name"]
        )
    )

    # Update leaderboard
    # Check if user(s) are already in leaderboard
    user1 = _leaderboard.find_leaderboard_user_by_id(
        db_guild, db_challenge["player1"]["id"]
    )
    if not user1:
        # Create new leaderboard user
        await _leaderboard.create_leaderboard_user(
            challenge_message.guild,
            db_challenge,
            db_challenge["player1"],
            winner_emote == "1️⃣",
        )
        ...
        await _leaderboard.update_leaderboard_user_score(
            challenge_message.guild.id, db_challenge, user1, winner_emote == "1️⃣"
        )

    user2 = _leaderboard.find_leaderboard_user_by_id(
        db_guild, db_challenge["player2"]["id"]
    )
    if not user2:
        await _leaderboard.create_leaderboard_user(
            challenge_message.guild,
        ...
        # Update existing leaderboard user
        await _leaderboard.update_leaderboard_user_score(
            challenge_message.guild.id, db_challenge, user2, winner_emote == "2️⃣"
        )
    return True


async def override_challenge_result(
    interaction: Interaction, challenge_id: int, winner: str
):
    ...
    Args:
        interaction (Interaction): The discord command interaction.
        challenge_id (int): The target challenge id.
        winner (str): The winner of the challenge. Either the name of the winner, '1️⃣' or '2️⃣'.
    ...
    if not user.guild_permissions.administrator:
        await interaction.followup.send(
            f"Only the author or server admins can override challenge results.",
            ephemeral=True,
        )
        return False
    ...
    try:
        ...
        await interaction.followup.send(
            f"Something went wrong when finding the challenge.", ephemeral=True
        )
        return False
    if not db_challenge:
    ...
    if not db_challenge["accepted"]:
        await interaction.followup.send(
            f"This challenge has not been accepted yet.", ephemeral=True
        )
        return False
    ...
    if not winner_emote:
        ...
        if player1["name"].lower() == winner.lower():
            ...
            await interaction.followup.send(
                f"There is no entrant named '{winner}' in this challenge.\n{usage}",
                ephemeral=True,
            )
            return False
    # Check if actually changing the winner
    if (
        db_challenge["winner_emote"] is not None
        and winner_emote == db_challenge["winner_emote"]
    ):
        await interaction.followup.send(
            "Challenge report failed; Winner is the same.", ephemeral=True
        )
        return False
    # Report match
    ...
    try:
        ...
        return False
    printlog(
        f"User ['name'='{user.name}'] overwrote result for challenge ['id'='{db_challenge['id']}']. Winner: {winner['name']} {winner_emote}."
    )
    await interaction.followup.send(
        content=f"Challenge report successful. Winner: {winner['name']} {winner_emote}"
    )
    return True
...
def create_challenge_embed(db_challenge: dict) -> Embed:
    """Creates the embed object to include in challenge message.

    Args:
        db_challenge (dict): The target challenge database document.

    Returns:
        Embed: The result Embed instance.
    """
    ...
    # Main embed
    embed = Embed(
        title=f"⚔️  Challenge - Best of {db_challenge['best_of']}",
        description=f"{player1_name} has issued a challenge!",
        color=WOOP_BLUE,
    )
    # Author field
    embed.set_author(
    ...
    if not player2_id:
        embed.add_field(
            name=f"Waiting for Challenger...",
            value=f"1️⃣ <@{player1_id}> vs ??? 2️⃣",
            inline=False,
        )
    else:
        embed.add_field(
    ...
    # Challenge footer
    embed.set_footer(
        text=f"Created by {player1_name}\nchallenge_id: {db_challenge['id']}."
    )
    return embed


def edit_challenge_embed_start(embed: Embed, db_challenge: dict) -> Embed:
    ...
    Returns:
        Embed: The updated Embed instance.
    """
    player1_id = db_challenge["player1"]["id"]
    player2_id = db_challenge["player2"]["id"]
...
def edit_challenge_embed_dispute(embed: Embed):
    ...
    Returns:
        _type_: The updated Embed instance.
    """
    embed.add_field(
        name="🛑 Result Dispute 🛑",
...
class accept_view(discord.ui.View):
    ...
    @discord.ui.button(
        label="Cancel", style=discord.ButtonStyle.danger, custom_id="cancel_challenge"
    )
    async def Cancel(
        self: discord.ui.View,
        interaction: discord.Interaction,
...
class voting_buttons_view(discord.ui.View):
    ...
    @discord.ui.button(
        emoji="1️⃣", style=discord.ButtonStyle.grey, custom_id="vote_player1"
    )
    async def vote_player1(
        self: discord.ui.View,
        interaction: discord.Interaction,
        button: discord.ui.Button,
    ):
    ...
    @discord.ui.button(
        emoji="2️⃣", style=discord.ButtonStyle.grey, custom_id="vote_player2"
    )
    async def vote_player2(
        self: discord.ui.View,
        interaction: discord.Interaction,
        button: discord.ui.Button,
    ):
...
async def retrieve_valid_challenge(
    ...
    If challenge_id is not provided, tries to find an active challenge from the user who made the interaction.

    Args:
        interaction (Interaction): The Discord command interaction.
        db_guild (dict): The guild database document.
        challenge_id (int, optional): The target challenge id. Defaults to None.
        send (bool, optional): A flag to denote whether to respond to the interaction. Defaults to True.

    Returns:
        A pair of the challenge document and the challenge id.
    """
    ...
    if challenge_id:
        ...
        if not db_challenge:
            if send:
                await interaction.followup.send(
                    f"You do not have any open challenges.", ephemeral=True
                )
            return (None, None)
        challenge_id = db_challenge["id"]
...
async def set_challenge(guild_id: int, challenge_id: int, new_challenge: dict):
    ...
    Args:
        ...
        challenge_id (int): The target challenge id.
        new_challenge (dict): The new challenge document.

    Returns:
        The new challenge document if successful. Otherwise, None.
    """
...
async def update_player(
    db_guild: dict,
    challenge_id: int,
    updated_player1: dict = None,
    updated_player2: dict = None,
):
    ...
    Args:
        db_guild (dict): The guild database document.
        challenge_id (int): The target challenge id.
        updated_player1 (dict, optional): The updated player1 document. Defaults to None.
        updated_player2 (dict, optional): The updated player2 document. Defaults to None.
