...
async def add_guild(db: Database, guild: Guild):
    ...
    # Add to database
    document_id = await mdb.add_document(db, new_guild, GUILDS)
    if document_id:
...
async def update_guild(db: Database, guild: Guild):
    ...
    """
    db_guild = await mdb.update_single_document(db, {'guild_id': guild.id}, {'$set': {'name': guild.name}}, GUILDS)
    if db_guild:
...
async def delete_guild(db: Database, guild: Guild):
    ...
    """
    delete_result = await mdb.delete_document(db, {'guild_id': guild.id}, GUILDS)
    if delete_result:
