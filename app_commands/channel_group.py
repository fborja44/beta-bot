...
ChannelGroup = app_commands.Group(
    name="ch",
    description="Channel configuration commands.",
    guild_ids=[133296587047829505, 713190806688628786],
    guild_only=True,
)
...
@ChannelGroup.command(
    description="Creates a tournament channel. Forum Channels are recommended if available."
)
async def create(
    ...
    await channel.create_tournament_channel(
        interaction, channel_name.strip(), target_category.strip(), is_forum
    )


@ChannelGroup.command(
    description="Sets a channel to be a tournament channel. Forum Channels are recommended if available."
)
...
@ChannelGroup.command(
    description="Removes a channel as tournament channel without deleting it."
)
...
@ChannelGroup.command(
    description="Recreates the management thread in a tournament forum channel."
)
...
@ChannelGroup.command(
    description="Lists all channels receiving alerts from the target tournament channel."
)
...
async def remove_alert(
    ...
    await interaction.response.defer()
    await channel.remove_channel_from_alerts(
        interaction, tournament_channel, alert_channel
    )
