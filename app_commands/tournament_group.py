...
TournamentGroup = app_commands.Group(
    name="bracket",
    description="Tournament bracket commands.",
...
@TournamentGroup.command(
    description="Creates a tournament bracket. Times in ET. Default: double elimination."
)
async def create(
    ...
    max_participants: int = MAX_ENTRANTS,
    as_thread: bool = False,
):
    await interaction.response.defer(ephemeral=True)
    await tournament.create_tournament(
        interaction,
        title.strip(),
        time.strip(),
        single_elim,
...
@TournamentGroup.command(
    description="Sets the seed for a participant in a tournament bracket."
)
...
async def update(
    ...
    await interaction.response.defer(ephemeral=True)
    await tournament.update_tournament(
        interaction, title, new_title, time, single_elim, max_participants
    )
...
@TournamentGroup.command(
    description="Sends the results for a completed tournament bracket."
)
...
@TournamentGroup.command(
    description="Disqualifies or removes an participant from a tournament bracket."
)
