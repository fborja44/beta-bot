...
ChallengeGroup = app_commands.Group(
    name="challenge",
...
@ChallengeGroup.command(
    description="Creates a direct challenge to the mentioned player."
)
...
@ChallengeGroup.command(
    description="Cancels a challenge that has not yet been completed."
)
async def cancel(interaction: Interaction, challenge_id: str | None = None):
    if not challenge_id.isnumeric():
        await interaction.response.send_message(
            "`challenge_id` must be a valid integer.", ephemeral=True
        )
        return False
...
@ChallengeGroup.command(
    description="[Privileged] Manually reports the result for a challenge.."
)
async def report(interaction: Interaction, challenge_id: str, winner: str):
    if not challenge_id.isnumeric():
        ...
        return False
    await interaction.response.defer()
    await challenge.override_challenge_result(
        interaction, int(challenge_id), winner.strip()
    )
